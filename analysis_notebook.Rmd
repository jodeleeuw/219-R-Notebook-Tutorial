...
```{r}
behavioral.data <- read.csv('data/behavioral_data_tidy.csv')
```
...
load('data/voltage_data.tidy.Rdata')
```
...
grand.average.data <- all.data %>% filter(electrode %in% target.electrodes) %>% 
...
anova.data <- all.data %>% filter(electrode %in% target.electrodes, t %in% n400.window) %>%
